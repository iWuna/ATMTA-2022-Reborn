...
/obj/machinery/chem_dispenser/scp_294/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open=0)
	...
	if (!ui)
		...
		ui = new(user, src, ui_key, "chem_dispenser.tmpl", "[src.name]", 390, 315)
	...
	if(href_list["input"])
		if(beaker)
			...
			if (input_reagent in prohibited_reagents)
				...
				try
					U.add_reagent(input_reagent, min(amount, amount * 10, space))
					amount = max(amount - min(amount, amount * 10, space) / 10, 0)
				catch
					visible_message("OUT OF RANGE")
